        ...
        <div class="ai-header-card p-4 p-md-5 rounded-5 shadow-lg position-relative overflow-hidden text-white">
          ...
          <div class="position-relative z-1 d-md-flex align-items-center justify-content-between">
            <div>
              ...
              <h1 class="display-5 fw-bold mb-2">Análisis Científico <span class="text-warning">Diario</span></h1>
              <p class="lead opacity-75 mb-0" style="max-width: 600px;">
                Descubre conexiones ocultas, fronteras del conocimiento y vacíos de investigación mediante inferencia generativa sobre la producción científica diaria.
              </p>
            </div>
            ...
            <div class="mt-4 mt-md-0 ms-md-5 p-4 rounded-4 bg-black bg-opacity-25 border border-white border-opacity-10 backdrop-blur-sm shadow-sm" style="max-width: 500px;">
                <h6 class="fw-bold mb-3 text-warning d-flex align-items-center">
                  <i class="bi bi-activity me-2"></i>El Pulso Vivo de la Ciencia
                </h6>
                <p class="small mb-2 opacity-75 lh-sm">
                    A diferencia de los reportes estáticos, este análisis diario actúa como un <strong>monitor en tiempo real</strong>:
                </p>
                <ul class="mb-3 ps-3 small opacity-75">
                    <li class="mb-2">
                        <strong>Daily Connections:</strong> Maximiza la <strong>serendipia</strong>, identificando cruces creativos únicos en la producción de las últimas 24h.
                    </li>
                    <li class="mb-0">
                        <strong>Frontiers & Gaps:</strong> Son <strong>"Snapshots"</strong> que permiten detectar <em>señales débiles</em> y micro-tendencias emergentes mucho antes de que aparezcan en las estadísticas globales.
                    </li>
                </ul>
                <div class="border-top border-white border-opacity-10 pt-2 mt-2">
                    <p class="mb-0 text-white text-opacity-50 fst-italic" style="font-size: 0.75rem; line-height: 1.4;">
                        <i class="bi bi-exclamation-circle me-1"></i> Basado en una muestra aleatoria de 100 documentos de Crossref publicados en este día concreto. Debido al muestreo limitado, es posible que ciertos temas relevantes no sean capturados en la instantánea.
                    </p>
                </div>
            </div>
          </div>
        ...
        <div class="row g-0 mb-4 rounded-4 overflow-hidden shadow-sm border bg-white nav-tabs-container">
          <div class="col-md-4">
            ...
            >
              ...
              <span class="fw-bold h5 mb-0">Daily Connections</span>
          ...
          <div class="col-md-4">
            ...
            >
              ...
              <span class="fw-bold h5 mb-0">Frontiers</span>
          ...
          <div class="col-md-4">
            ...
            >
              ...
              <span class="fw-bold h5 mb-0">Knowledge Gaps</span>
        ...
        <div class="info-card p-4 rounded-4 d-flex align-items-center gap-4 mb-5" :class="infoCardClass">
            <div class="d-flex align-items-start gap-4 w-100">
              ...
              <div>
                ...
                <p class="mb-0 opacity-75 small" style="line-height: 1.5;">{{ currentDescription }}</p>
              </div>
            </div>
        </div>
        ...
        <div class="row align-items-center justify-content-between mb-4 px-2">
            <div class="col-md-auto d-flex align-items-center gap-3">
                ...
                <h2 class="h3 fw-bold mb-0 text-dark">
                    Análisis del día: <span class="text-warning">{{ selectedDate }}</span>
                </h2>
            </div>
            ...
            <div class="analysis-content-wrapper p-0" v-loading="loading[activeTab]">
              ...
              <div v-else-if="!loading[activeTab]" class="text-center py-5">
                <div class="alert alert-danger rounded-4 py-5 shadow-sm border-0 animate__animated animate__shakeX">
                  ...
                  <h4 class="fw-bold text-danger mb-2">Análisis no disponible</h4>
                  <p class="mb-0 text-secondary">No se ha generado o no hay datos suficientes para realizar el análisis de esta categoría en la fecha seleccionada.</p>
                </div>
              </div>
              ...
              <div class="mt-5 pt-4 border-top">
                <div class="d-flex justify-content-center">
                  ...
                  >
                    ...
                    {{ showSamples ? 'Ocultar fuentes analizadas' : 'Ver fuentes del análisis (100 artículos)' }}
                  ...
                  <div class="table-responsive rounded-4 border shadow-sm bg-light bg-opacity-50">
                    <table class="table table-hover align-middle mb-0" style="font-size: 0.85rem;">
                      <thead class="table-dark">
                        <tr>
                          <th scope="col" class="ps-4">Título / DOI</th>
                          <th scope="col" class="text-end pe-4">Publicador</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr v-for="work in dailySamples" :key="work.doi">
                          <td class="ps-4 py-3" style="max-width: 500px;">
                            <div class="fw-bold text-dark mb-1">{{ work.title || 'Sin título' }}</div>
                        ...
                        <tr v-if="dailySamples.length === 0 && !loadingSamples">
                          <td colspan="2" class="text-center py-4 text-muted">No se encontraron datos de muestreo para esta fecha.</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const toggleSamples = async () => {
      ...
      if (dailySamples.value.length === 0) {
        ...
        try {
          ...
          ElMessage.error('Error al cargar las fuentes del análisis.');
          console.error(error);
        } finally {
    ...
    const currentTitle = computed(() => {
      const titles = {
        connection: 'Daily Connections (Puentes Interdisciplinares)',
        frontier: 'Frontiers (Términos Vanguardistas)',
        gap: 'Knowledge Gaps (Puntos Ciegos)'
      };
    ...
    const currentDescription = computed(() => {
      const descs = {
        connection: 'Este análisis identifica puentes interdisciplinares emergentes, revelando cómo conceptos de diferentes campos científicos comienzan a entrelazarse y crear nuevas sinergias en las publicaciones más recientes.',
        frontier: 'Detecta los términos, metodologías y conceptos más novedosos que están marcando tendencia en la literatura científica actual, señalando hacia dónde se mueve la frontera del conocimiento.',
        gap: 'Localiza áreas sub-exploradas o "vacíos" en la red de conocimiento actual donde la densidad de investigación es baja, sugiriendo oportunidades valiosas para nueva investigación científica.'
      };
...
/* Custom Scrollbar for the analysis content */
/* analysis-content-wrapper styles removed to allow natural page scrolling */
